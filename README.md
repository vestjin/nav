...
| 书签体积上限 | Gist 文件 1MB / 卡片几百项无压力 | 超出需切 raw_url 拉取，已实现 |
...
1. **PWA** — 加 manifest + service worker，支持安装到桌面、完全离线
2. **多 Gist 切换** — 工作/生活/学习 多个 Gist
3. **拖拽排序** — `@dnd-kit` 或原生 HTML5 drag（增加 ~10KB）
4. **暗色模式** — 加 `@media (prefers-color-scheme: dark)` 切换
5. **导入 HTML 书签** — 解析 Chrome/Firefox 导出的 bookmarks.html
6. **快捷键直达** — 输入 `gh` 回车直达 GitHub
7. **图标本地缓存** — 把抓到的 favicon 转 base64 存 localStorage
8. **分组折叠** — 分类可收起/展开
